class LEXICON_RU:
    start: str = ('<b>Привет!</b>\nДавай с тобой сыграем в игру '
                  '"Камень, ножницы, бумага"?\n\nЕсли ты, вдруг, '
                  'забыл правила, команда /help тебе поможет!\n\n<b>Играем?</b>')

    help: str = ('Это очень простая игра. Мы одновременно должны '
                 'сделать выбор одного из трех предметов. Камень, '
                 'ножницы или бумага.\n\nЕсли наш выбор '
                 'совпадает - ничья, а в остальных случаях камень '
                 'побеждает ножницы, ножницы побеждают бумагу, '
                 'а бумага побеждает камень.\n\n<b>Играем?</b>')

    rock: str = 'Камень 🗿'
    paper: str = 'Бумага 📜'
    scissors: str = 'Ножницы ✂'
    yes_button: str = 'Давай!'
    no_button: str = 'Не хочу!'
    other_answer: str = 'Извини, увы, это сообщение мне непонятно...'
    yes: str = 'Отлично! Делай свой выбор!'
    no: str = ('Жаль...\nЕсли захочешь сыграть, просто разверни '
               'клавиатуру и нажми кнопку "Давай!"')
    bot_won: str = 'Я победил!\n\nСыграем еще?'
    user_won: str = 'Ты победил! Поздравляю!\n\nДавай сыграем еще?'
    nobody_won: str = 'Ничья!\n\nПродолжим?'
    bot_choice: str = 'Мой выбор'


